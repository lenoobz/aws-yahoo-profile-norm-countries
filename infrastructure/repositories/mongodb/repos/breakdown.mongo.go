...
import (
	...
	"github.com/lenoobz/aws-yahoo-profile-norm-countries/infrastructure/repositories/mongodb/models"
...
// NewCountryBreakdownMongo creates new country breakdown mongo repo
func NewCountryBreakdownMongo(db *mongo.Database, log logger.ContextLog, conf *config.MongoConfig) (*CountryBreakdownMongo, error) {
	if db != nil {
...
// InsertAssetCountryBreakdown adds new asset country breakdown
func (r *CountryBreakdownMongo) InsertAssetCountryBreakdown(ctx context.Context, breakdown *entities.FundBreakdown) error {
	// create new context for the query
	ctx, cancel := createContext(ctx, r.conf.TimeoutMS)
	defer cancel()

	countryBreakdownModel, err := models.NewFundBreakdownModel(ctx, r.log, breakdown, r.conf.SchemaVersion)
	if err != nil {
	...
	filter := bson.D{{
		Key:   "ticker",
		Value: countryBreakdownModel.Ticker,
	}}

	update := bson.D{
		{
			Key:   "$set",
			Value: countryBreakdownModel,
		},
		{
			Key: "$setOnInsert",
			Value: bson.D{{
